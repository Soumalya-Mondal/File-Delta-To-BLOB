...
def file_download_from_blob(env_file_path: str, files_delta_store_folder_path: str, file_hash_download: bool = False) -> dict[str, str]: #type: ignore
    ...
    try:
        if Path(env_file_path).exists() and Path(env_file_path).is_file():
            ...
            return {'status': 'ERROR', 'script_name': 'BLOB-Files-Operation', 'step': '3', 'message': 'Environment File Not Found'}
    except Exception as error:
...
def files_clear_from_blob(env_file_path: str, hash_file_delete: bool = False) -> dict[str, str]: #type: ignore
    # Importing Python Module:S1
    try:
        from pathlib import Path
        from dotenv import load_dotenv
    ...
    # Verify All BLOBs Deleted Based On hash_file_delete Parameter:S5
    try:
        ...
        if hash_file_delete:
            if len(remaining_blobs) == 0:
                ...
                return {'status': 'SUCCESS', 'script_name': 'BLOB-Files-Operation', 'step': '5', 'message': 'Container Cleared Successfully Including "FileHashDetails.json"'}
            else:
                return {'status': 'ERROR', 'script_name': 'BLOB-Files-Operation', 'step': '5', 'message': f'Unexpected BLOBs Still Present: {", ".join(Path(blob).name for blob in remaining_blobs)}'}
        else:
            if (len(remaining_blobs) == 0) or (len(remaining_blobs) == 1 and remaining_blobs[0] == 'FileHashDetails.json'):
                ...
                return {'status': 'SUCCESS', 'script_name': 'BLOB-Files-Operation', 'step': '5', 'message': 'Container Cleared Successfully Except "FileHashDetails.json"'}
            else:
                return {'status': 'ERROR', 'script_name': 'BLOB-Files-Operation', 'step': '5', 'message': f'Unexpected BLOBs Still Present: {", ".join(Path(blob).name for blob in remaining_blobs)}'}
    except Exception as error:
        return {'status': 'ERROR', 'script_name': 'BLOB-Files-Operation', 'step': '5', 'message': str(error)}
